...
lift will provision missing images and run all targeted containers.`,
		Run: configCommand(func(config Config) {
			config.TargetedContainers().lift(config.ContainerMap(), options.recreate, options.nocache, options.ignoreMissing)
		}, false),
	}
...
If no Dockerfile is given, it will pull the image(s) from the given registry.`,
	...
	var cmdCreate = &cobra.Command{
		...
		Run: configCommand(func(config Config) {
			config.TargetedContainers().create(config.ContainerMap(), options.recreate, options.ignoreMissing)
		}, false),
	}
	...
	var cmdRun = &cobra.Command{
		...
		Run: configCommand(func(config Config) {
			config.TargetedContainers().run(config.ContainerMap(), options.recreate, options.ignoreMissing)
		}, false),
	}
